// uncover_markdown_generator.ts 的测试用例
import main from '../../script/markdown/uncover_markdown_generator';
import { meituanTestData } from '../testData';

describe('uncover_markdown_generator.ts - 未覆盖订单数据Markdown生成', () => {
  ...
  test('应该处理未知支付方式', async () => {
    const input = [
      { ...meituanTestData[0], 支付方式: '' },
      { ...meituanTestData[0], 支付方式: null as any },
    ];
    const result = await main({ params: { input } });

    expect(result.output).toBeDefined();
    expect(result.output).toContain('## 未知支付方式');
  });
});
